###################################################################
####Author:  Noffisat Oki, modified by Tatyana Doktorova
####Version: 1.0 
####Description: automated extraction of TG Gates data using Api
#####################################################################
...
###Getting  compounds present in TGG
Ste_chem_data <- read.table(paste(Root_location,"/InputFiles/CASsteatosis.txt",sep=''),sep='\t',header=T,comment.char='',strip.white=T)
newcat <- which(caslist2$Casrn %in% Ste_chem_data$Cas.number)
newcat2 <- caslist2[newcat,]
TGG_in_Ste_list <- unique(merge(caslist2, Ste_chem_data,  by.x="Casrn", by.y="Cas.number" ,all.x=FALSE, all.y=FALSE))
TGGinSte_compoundset <- "/OutputFiles/TGG_in_Ste_compounds.txt"
write.table(TGG_in_Ste_list, paste(Root_location,TGGinSte_compoundset,sep=''),sep="\t",row.names=F)
...
Ste_samples_in_TGG <- unique(merge(newcat2, Samples, by.x ="TGG_compoundName", by.y = "compoundName" ))
Ste_in_TGG_sampleset <- "/OutputFiles/TGG_in_Ste_samples.txt"
write.table(Ste_samples_in_TGG[,c(1:11)], paste(Root_location,Ste_in_TGG_sampleset,sep=''),sep="\t",row.names=F)

sampleIDs <- unique(Ste_samples_in_TGG$sampleId)
sampleIDs_human <- Ste_samples_in_TGG[c(which(Ste_samples_in_TGG$organism =='Human')),]$sampleId  #&& Ste_samples_in_TGG$tissue =='Liver')),]
sampleIDs_rat <- Ste_samples_in_TGG[c(which(Ste_samples_in_TGG$organism =='Rat')),]$sampleId
...
rat_dataset1 <- "/OutputFiles/TGG_data_absolute_rat_Steatotic.txt"
rat_dataset2 <- "/OutputFiles/TGG_data_log2fold_Steatotic_rat.txt"
